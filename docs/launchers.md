...
A table's **Settings** section (`console/app_settings.py`) comes after **Table** on its rail. It holds the launcher that plays the table and **Clear NVRAM on Exit**, then the program's settings for this table - the table's differences and nothing else: what its own file sets and what reaches it from the game's, by area, with the camera as one row. With none it says *Same as all tables*, and **Show Every Setting** opens All Settings at this table. A value set equal to what all tables use clears the table's own instead. A setting the table's file holds that the program reads only for all tables stays listed as *Unused*, read-only, with **Clear**. **Launch** reports the launcher in one line, with how many settings the table has of its own or from its game, as a link to the section.
