...
| GET | `/api/v1/games/{id}/tables` | The game's tables, with resolved assets and dependencies. `update_available` is true where VPS lists a later version of the release a table is matched to than the file's own `version`, false where it does not, and null where nothing can be weighed - no release, or a version on either side that does not read as one. `launcher_settings_here` and `launcher_settings_from_folder` count the settings the program's file for the table changes, under whichever of the two files it is, `launcher_settings_keys` lists which by the program's own key, the camera left out, and `launcher_point_of_view` says whether that file holds a camera |
| GET | `/api/v1/tables` | Every table in the library (`game`, `limit`, `offset`), each row carrying `source`, the named release, `update_available`, `launcher_settings_here`, `launcher_settings_from_folder`, `launcher_settings_keys`, `launcher_point_of_view` and `launcher_app_configurable` as a game's tables do, and `derived_tags`. The launcher counts read each table's settings file |
...
| GET | `/api/v1/extensions` | Every extension this install looked at, running or not, and what each declared. `enabled` is the switch, which is not the same as running. One that is not running carries `reason`, in this install's language, and `reason_key`, the catalog key it was read from |
| PUT | `/api/v1/extensions/{name}/enabled` | Switch one on or off, `{"enabled": bool}`, written to `extensions.json`. Answers with the extension. Off takes it out at once, as a fault would, and its `reason_key` says it was switched off. On waits for the next start, because routes mount once: until then it stays `off`, with `extension.reason.starts_at_restart`, or the reason this install cannot run it |
| any | `/api/v1/ext/{name}/…` | An extension's own routes, gated on a scope it declared. `501` once it has stopped or been switched off, naming which one and why. One that never started, switched off before this start or not for this install, has no routes, so `404`. See `docs/extensions.md` |
