...
| GET | `/api/v1/games/{id}/tables` | The game's tables, with resolved assets and dependencies. `update_available` is true where VPS lists a later version of the release a table is matched to than the file's own `version`, false where it does not, and null where nothing can be weighed - no release, or a version on either side that does not read as one. `launcher_settings_here` and `launcher_settings_from_folder` count the settings the program's file for the table changes, under whichever of the two files it is, and `launcher_point_of_view` says whether that file holds a camera |
| GET | `/api/v1/tables` | Every table in the library (`game`, `limit`, `offset`), each row carrying `source`, the named release, `update_available`, `launcher_settings_here`, `launcher_settings_from_folder`, `launcher_point_of_view` and `launcher_app_configurable` as a game's tables do, and `derived_tags`. The launcher counts read each table's settings file |
